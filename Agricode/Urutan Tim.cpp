...
public:
	...
	inline int get_nilai(){
		return nilai;
	}
	inline int get_waktu(){
		return waktu;
	}
	friend ostream &operator<<(ostream &out,const peserta& p){
		out<<p.nama;
		return out;
	}
	friend istream &operator>>(istream &in,peserta &p){
		in>>p.nama>>p.nilai>>p.waktu;
		return in;
	}
};

inline bool cmp(peserta a,peserta b){
	if(a.get_nilai()==b.get_nilai())
		return a.get_waktu()<b.get_waktu();
	return a.get_nilai()>b.get_nilai();
}
