...
int main(int argc, char **argv){
	...
	string perm[6]={"BCG","BGC","CBG","CGB","GBC","GCB"};
	int a[9],mn,mo[6];
	while(cin>>a[0]){
		...
		// Clear : 2 5 8
		//6 Permutations:
		mo[0]=a[1]+a[2]+a[3]+a[4]+a[6]+a[8];
		mo[1]=a[1]+a[2]+a[3]+a[5]+a[6]+a[7];
		mo[2]=a[0]+a[1]+a[4]+a[5]+a[6]+a[8];
		mo[3]=a[0]+a[1]+a[3]+a[5]+a[7]+a[8];
		mo[4]=a[0]+a[2]+a[4]+a[5]+a[6]+a[7];
		mo[5]=a[0]+a[2]+a[3]+a[4]+a[7]+a[8];
		
		mn=*min_element(mo,mo+6);
		for(int i=0;i<6;++i){
			if(mo[i]==mn){
				cout<<perm[i]<<' '<<mn<<'\n';
				break;
